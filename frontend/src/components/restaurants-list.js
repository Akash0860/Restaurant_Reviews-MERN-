...
const RestaurantsList = props => {
  ...
  const [cuisines, setCuisines] = useState([{id: 0, name: "All Cuisines"}]);
  ...
  const retrieveCuisines = () => {
    RestaurantDataService.getCuisines()
      .then(response => {
        const cuisinesWithIds = response.data.map((cuisine, index) => {
          return { id: index + 1, name: cuisine };
        });
        setCuisines([{ id: 0, name: "All Cuisines" }, ...cuisinesWithIds]);
      })
      .catch(e => {
        console.log(e);
      });
